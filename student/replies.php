...
$root_path = "../";
$page_title = "Ticket Chat | Student Portal";
...
// Fetch Student details for Audit
...
$stmt = $pdo->prepare("
    ...
    WHERE t.id = ? AND t.user_id = ? AND t.user_type = 'STUDENT'
");
    ...
    if (!empty($message_content)) {
        try {
            $stmt_reply = $pdo->prepare("
                ...
                VALUES (?, ?, 'STUDENT', ?)
            ");
            $stmt_reply->execute([$ticket_id, $student_id, $message_content]);
...
<div class="main-dashboard text-start" style="padding: 20px 0;">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Follow the responses from school administrators and reply to this ticket thread.</p>
        </div>
                ...
                <div class="card-body flex-grow-1 p-4" style="background-color: #fafbfc; max-height: 400px; overflow-y: auto;">
                    <?php if (count($replies) > 0): ?>
                        <?php foreach($replies as $rep): ?>
                            <?php
                            $is_self = $rep['user_type'] === 'STUDENT' && $rep['user_id'] == $student_id;
                            ?>
