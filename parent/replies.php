...
$root_path = "../";
$page_title = "Ticket Chat | Parent Portal";
...
// Fetch Parent name
$stmt_par = $pdo->prepare("SELECT parent_name FROM parents WHERE id = ?");
$stmt_par->execute([$parent_id]);
$parent = $stmt_par->fetch(PDO::FETCH_ASSOC);
$parentName = $parent['parent_name'] ?? 'Parent';

// Fetch Ticket and ensure it belongs to the logged in parent
$stmt = $pdo->prepare("
    SELECT t.*, tc.category_name, a.name as assignee_name
    FROM tickets t
    LEFT JOIN ticket_categories tc ON t.category_id = tc.id
    LEFT JOIN admins a ON t.assigned_to = a.id
    WHERE t.id = ? AND t.user_id = ? AND t.user_type = 'PARENT'
");
$stmt->execute([$ticket_id, $parent_id]);
    ...
    if (!empty($message_content)) {
        try {
            $stmt_reply = $pdo->prepare("
                ...
                VALUES (?, ?, 'PARENT', ?)
            ");
            $stmt_reply->execute([$ticket_id, $parent_id, $message_content]);
...
<div class="main-dashboard text-start" style="padding: 20px 0;">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Follow updates from the administration regarding this ticket thread.</p>
        </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 p-4" style="border-radius: 12px; height: 100%;">
                <h5 class="fw-bold text-dark mb-3 border-bottom pb-2 text-primary" style="color: #7c3aed !important;"><i class="fa fa-info-circle me-2"></i>Ticket Metadata</h5>
                ...
                <div class="mb-3">
                    ...
                    <span class="badge bg-primary px-2 py-1" style="background-color: #7c3aed !important;"><?= htmlspecialchars($ticket['priority']) ?></span>
                </div>
                ...
                <div class="card-body flex-grow-1 p-4" style="background-color: #fafbfc; max-height: 400px; overflow-y: auto;">
                    <?php if (count($replies) > 0): ?>
                        <?php foreach($replies as $rep): ?>
                            <?php
                            $is_self = $rep['user_type'] === 'PARENT' && $rep['user_id'] == $parent_id;
                            ?>
                            <div class="d-flex mb-3 <?= $is_self ? 'justify-content-end' : 'justify-content-start' ?>">
                                <div class="p-3 rounded shadow-sm border" style="max-width: 70%; border-radius: 15px !important; background-color: <?= $is_self ? '#f5f3ff' : '#ffffff' ?>;">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <span class="small fw-bold <?= $is_self ? 'text-primary' : 'text-dark' ?>" style="<?= $is_self ? 'color: #7c3aed !important;' : '' ?>"><?= htmlspecialchars($rep['sender_name'] ?: 'User') ?></span>
                ...
                <div class="card-footer bg-white border-0 p-4">
                    <?php if ($ticket['status'] !== 'CLOSED'): ?>
                        <form method="POST">
                            ...
                            <div class="text-end">
                                <button type="submit" name="post_reply" class="btn btn-primary px-4 fw-semibold" style="background-color: #7c3aed; border-color: #7c3aed;">
