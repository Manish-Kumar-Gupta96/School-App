...
$root_path = "../../";
$page_title = "Ticket Conversation | Helpdesk Control";
    ...
    if (!empty($message_content)) {
        try {
            $stmt_reply = $pdo->prepare("
                ...
                VALUES (?, ?, 'ADMIN', ?)
            ");
            $stmt_reply->execute([$ticket_id, $_SESSION['user_id'], $message_content]);
...
<div class="main-dashboard text-start">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">💬 Ticket Thread: <?= htmlspecialchars($ticket['ticket_no']) ?></h2>
            <p class="text-muted mb-0">Conversation thread regarding support inquiry.</p>
        </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 p-4" style="border-radius: 12px; height: 100%;">
                <h5 class="fw-bold text-dark mb-3 border-bottom pb-2 text-primary"><i class="fa fa-info-circle me-2"></i>Ticket Metadata</h5>
                ...
                <div class="mb-3">
                    ...
                    <span class="badge bg-primary px-2 py-1"><?= htmlspecialchars($ticket['priority']) ?></span>
                </div>
                ...
                <div class="mb-3">
                    <label class="text-muted small fw-bold uppercase d-block">Assigned To</label>
                    <span class="text-dark fw-bold"><i class="fa fa-circle-user text-muted me-1"></i> <?= htmlspecialchars($ticket['assignee_name'] ?: 'Unassigned') ?></span>
                </div>
                ...
                <?php if ($attachment): ?>
                    <div class="mb-3">
                        ...
                        <a href="attachments/<?= htmlspecialchars($attachment['file_name']) ?>" target="_blank" class="btn btn-sm btn-outline-success mt-2">
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 d-flex flex-column" style="border-radius: 12px; height: 100%; min-height: 500px;">
                <div class="card-header bg-white border-0 py-3 ps-4">
                    <h5 class="fw-bold mb-0 text-dark"><i class="fa fa-comments me-2 text-warning"></i>Communication History</h5>
                </div>
                ...
                <div class="card-body flex-grow-1 p-4" style="background-color: #fafbfc; max-height: 400px; overflow-y: auto;">
                    <?php if (count($replies) > 0): ?>
                        <?php foreach($replies as $rep): ?>
                            <?php
                            $is_self = $rep['user_type'] === 'ADMIN' && $rep['user_id'] == $_SESSION['user_id'];
                            ?>
                            <div class="d-flex mb-3 <?= $is_self ? 'justify-content-end' : 'justify-content-start' ?>">
                                <div class="p-3 rounded shadow-sm border" style="max-width: 70%; border-radius: 15px !important; background-color: <?= $is_self ? '#e0f2fe' : '#ffffff' ?>;">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <span class="small fw-bold <?= $is_self ? 'text-primary' : 'text-dark' ?>"><?= htmlspecialchars($rep['sender_name'] ?: 'User') ?></span>
                                        <span class="badge bg-light text-muted border ms-2" style="font-size: 0.65rem;"><?= htmlspecialchars($rep['user_type']) ?></span>
                                    </div>
                                    <div class="text-secondary small" style="white-space: pre-wrap;"><?= htmlspecialchars($rep['message']) ?></div>
                ...
                <div class="card-footer bg-white border-0 p-4">
                    <?php if ($ticket['status'] !== 'CLOSED'): ?>
                        <form method="POST">
                            <div class="mb-3">
                                <textarea name="message" class="form-control" rows="3" placeholder="Compose support response..." required></textarea>
                            </div>
                            <div class="text-end">
                                <button type="submit" name="post_reply" class="btn btn-primary px-4 fw-semibold">
                                    <i class="fa fa-reply me-1"></i> Send Reply
                                </button>
                            </div>
                        </form>
                    <?php else: ?>
